...
def clean_json(raw_data):
    try:
        ...
        if match:
            print("[parse_unknown.py] Cleaned JSON data successfully.")
        else:
            print("[parse_unknown.py] No JSON object found.")
        return match.group() if match else None
    except Exception as e:
        print(f"[parse_unknown.py] Error cleaning data: {e}")
        return None
...
def update_csv(input_file, output_csv):
    try:
        with open(input_file, 'r') as file:
            ...
            print("[parse_unknown.py] Reading input file...")

            data = json.loads(raw_data)
            devices = data['message']
            print("[parse_unknown.py] JSON data loaded.")
            ...
            try:
                ...
                print("[parse_unknown.py] Existing CSV data loaded.")
            except FileNotFoundError:
                print(f"[parse_unknown.py] CSV file {output_csv} not found. A new file will be created.")

            for device in devices:
                if device.get('model') != 'Mega23M12' and device.get('friendly_name') != 'Coordinator':
                    ...
                    if ieeeAddr:
                        ...
                        print(f"[parse_unknown.py] Device {ieeeAddr} added/updated.")
            ...
            with open(output_csv, mode='w', newline='') as file:
                ...
                for row in sorted(current_data.values(), key=lambda x: x['ieeeAddr']):
                    writer.writerow(row)
                print("[parse_unknown.py] CSV file updated.")

    except json.JSONDecodeError as e:
        print(f"[parse_unknown.py] Error decoding JSON from input file: {e}")

if __name__ == "__main__":
    args = parse_arguments()
    print("[parse_unknown.py] Script started.")
    update_csv(args.input, args.output)
    print("[parse_unknown.py] Script finished.")
