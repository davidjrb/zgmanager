...
def parse_arguments():
    ...
    parser.add_argument('--parse_unknown', action='store_true', help='Parse unknown devices instead of known devices (default: False)')
...
def start_parse_gw(input_file, output_csv, parse_unknown=False):
    if parse_unknown:
        subprocess.run(['python3', 'parse_unknown.py', '--input', input_file, '--output', output_csv])
    else:
...
def main():
    ...
    if args.parse_unknown:
        csv_file = f'data/{args.gateway}/{args.gateway}_unknown.csv'
    else:
        ...
        with open(output_file, 'r') as file:
            ...
            if content:  # Check if file is not empty
                if not args.skip_parse:
                    start_parse_gw(output_file, csv_file, args.parse_unknown)
                break
